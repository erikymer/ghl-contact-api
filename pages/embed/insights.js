...
<body>
  ...
  <script>
    ...
    const labels = Array.from({ length: dataPoints.length }, (_, i) => {
      const d = new Date(); d.setMonth(d.getMonth() - (dataPoints.length - 1 - i));
    ...
    if (dataPoints.length) {
      new Chart(document.getElementById('chart').getContext('2d'), {
        ...
        data: {
          labels,
          datasets: [{ label: "${cleanTitle}", data: dataPoints, borderColor: "#3498db", fill: false }]
        },
        options: { responsive: true, scales: { y: { beginAtZero: false } } }
      });
    } else {
      document.body.innerHTML += "<p style='color:red;'>❌ No data found.</p>";
    }
  </script>
</body>
</html>`;
